...
class MeetingItem {
  ...
  MeetingItem({required this.id, required this.title, required this.meetingTime, required this.conflicted}) {
    // Parse the meeting time (assuming date comes from MeetingDate)
    _parseTime();
  }

  factory MeetingItem.fromJson(Map<String, dynamic> json) => _$MeetingItemFromJson(json);
  ...
  void _parseTime() {
    try {
      ...
      _startTime = DateTime(now.year, now.month, now.day, timeOfDay.hour, timeOfDay.minute);
      _endTime = _startTime!.add(const Duration(minutes: 30)); // Default 30 min duration
    } catch (e) {
  ...
  bool isCurrent(DateTime now) => (_startTime?.isBefore(now) ?? false) && (_endTime?.isAfter(now) ?? false);
